 ...
 *   - https://x402.org/facilitator   (x402 testnet facilitator)
...
import { BASE_SEPOLIA_CHAIN_ID, X402_NETWORK } from './chains.js';
...
// Apply x402 payment middleware — routes that match the config require payment
app.use(
  paymentMiddleware(
    {
      'GET /api/yields': {
...
/**
 * GET /api/yields  — $0.001 USDC via x402
 *
 * Returns current yield APRs for the two demo yield sources.
 * In production: replace with live on-chain TVL queries or protocol APIs.
 */
