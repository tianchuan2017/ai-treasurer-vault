...
function MemoCard({
  ...
  return (
    <div className="p-4 border-b border-border last:border-0">
      ...
      {/* CFO memo in serif — these are financial documents */}
      ...
      <div className="flex items-center gap-2 mb-3 text-xs text-muted">
        ...
        <div className="flex-1 h-1.5 rounded-full bg-surface2 overflow-hidden">
          <div className="h-full bg-indigo rounded-full" style={{ width: `${allocA}%` }} />
        </div>
        <span>B {allocB}%</span>
      </div>
...
function Dashboard() {
  ...
  const daysUntilPayday = nextPaydayTs
    ? Math.max(0, Math.ceil((Number(nextPaydayTs) * 1000 - Date.now()) / 86_400_000))
    : null;

  // Demo-static memos — in production: viem getLogs on CFOMemoEmitted
  const demoMemos = [
    {
      memoText:
    ...
    <div className="min-h-screen bg-bg">
        ...
        <div className="flex items-center gap-3">
          ...
          {user && (
            <div className="bg-surface2 border border-border rounded-full px-3 py-1 text-xs font-mono">
              {shortenAddr(activeWallet?.address ?? user.id.slice(0, 28))}
            </div>
          )}
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
          <StatCard label="Total Pool"   value={formatUSDC(totalAssets)} sub="USDC deposited" />
          <StatCard label="Yield Earned" value="$247" sub="this cycle · 14 days" color="text-emerald" />
          <StatCard label="Current APY"  value="8.7%" sub="blended across sources" />
          <StatCard
            label="Next Payday"
        ...
        <div className="bg-surface border border-border rounded-xl p-5 mb-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-muted bg-surface2 px-3 py-1 rounded-full">
              Last rebalanced: 2h ago · memo #{memoCount !== undefined ? Number(memoCount) : '…'}
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-5">
          ...
          <div className="bg-surface border border-border rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-border flex items-center justify-between">
              ...
              <span className="text-xs text-muted bg-surface2 px-2 py-0.5 rounded-full">
                {memoCount !== undefined ? `${memoCount} memos on-chain` : 'loading...'}
              </span>
            </div>
          ...
          <div className="bg-surface border border-border rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-border flex items-center justify-between">
              ...
              <span className="text-xs text-muted bg-surface2 px-2 py-0.5 rounded-full">
                {empCount !== undefined ? `${empCount}` : '...'} active · {daysUntilPayday ?? '...'}d to payday
              </span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {[
                    { name: 'Alice Chen',   addr: '0xA1b2C3d4E5f6A7B8', salary: '$5,000' },
                    { name: 'Bob Martinez', addr: '0xE5f6A7B8C9D0E1F2', salary: '$3,500' },
                    { name: 'Carol Kim',    addr: '0xC9D0E1F2A3B4C5D6', salary: '$4,200' },
                  ].map((emp) => (
                    <tr key={emp.addr} className="border-b border-border last:border-0">
                      <td className="px-5 py-3">
                        <div className="text-sm font-medium">{emp.name}</div>
                        <div className="text-xs text-muted font-mono">{emp.addr}…</div>
                      </td>
        ...
        <div className="mt-8 pt-6 border-t border-border text-xs text-muted flex items-center justify-between">
          ...
          <div className="flex gap-4">
            ...
            <a
              href="https://github.com/you/ai-treasurer-vault"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-text transition-colors"
            >
